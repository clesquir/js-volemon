...
describe('AchievementListener#Snoozer', function() {
	...
	it('creates achievement if not created on player won if has been match point zero if user is host', function() {
		...
		listener.onPointTaken(new PointTaken(gameId, 0, false, 0, GAME_MAXIMUM_POINTS - 1));

		assert.equal(0, UserAchievements.find().count());
		listener.onPlayerWon(new PlayerWon(gameId, userId, GAME_MAXIMUM_POINTS, GAME_MAXIMUM_POINTS - 1));
	...
	it('creates achievement if not created on player won if has been match point zero if user is client', function() {
		...
		listener.onPointTaken(new PointTaken(gameId, 0, false, GAME_MAXIMUM_POINTS - 1, 0));
	...
	it('do not create achievement if not created if not gameId on player won', function() {
		...
		assert.equal(0, UserAchievements.find().count());
		listener.onPlayerWon(new PlayerWon(Random.id(5), userId, GAME_MAXIMUM_POINTS, GAME_MAXIMUM_POINTS - 1));
		assert.equal(0, UserAchievements.find().count());
	});
	...
	it('do not create achievement if not created if not userId on player won', function() {
		...
		assert.equal(0, UserAchievements.find().count());
		listener.onPlayerWon(new PlayerWon(gameId, Random.id(5), GAME_MAXIMUM_POINTS, GAME_MAXIMUM_POINTS - 1));
		assert.equal(0, UserAchievements.find().count());
	});
